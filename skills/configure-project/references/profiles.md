...
separate decision. Install the local Git-policy runtime for early feedback and
configure Git Flow (`main` stable, `dev` integration) unless the user explicitly
selects trunk.

## hardened

Use for sensitive, public, regulated, or multi-contributor repositories. Add
supported CodeQL analysis, stricter GitHub review rules, CODEOWNERS planning,
provenance recommendations, and stronger verification. Confirm availability and
plan restrictions before proposing remote controls.
...
`quality`, `ci`, `security`, `dependencies`, and `release`. `standard` includes
`git-policy`. Local hooks provide bypassable early feedback; required CI and
remote rules remain the shared authority.
